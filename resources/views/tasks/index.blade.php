...
@section('content')
    ...
    <div class="container">
        <div class="bg-white align-items-center mb-4 shadow-sm p-3 rounded">
            <h2 class="text-center">{{ $project->name ?? 'All Tasks' }}</h2>
        </div>
            ...
            @foreach ($statuses as $statusKey => $statusLabel)
                <div class="col-md-4">
                    <div class="kanban-column">
                        <div class="d-flex justify-content-between shadow-sm align-items-center
                            {{ $statusKey == 'to_do' ? 'bg-primary' : ($statusKey == 'in_progress' ? 'bg-warning' : 'bg-success') }}
                            text-white px-3 py-2 rounded-top">
                            <h4 class="text-white fw-bolder m-0">{{ $statusLabel }}</h4>

                            @if(isset($project))
                                <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#createTaskModal"
                                    data-status="{{ $statusKey }}" style="padding-top: 0.5rem; padding-bottom: 0.5rem;">+</button>
                            @endif
                        </div>

                        <div class="kanban-list" id="{{ $statusKey }}">
                                    ...
                                    <div class="card-body d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="d-flex gap-1">
                                            ...
                                            <a href="{{ route('tasks.show', $task->id) }}"
                                               class="btn btn-{{ $statusKey == 'to_do' ? 'primary' : ($statusKey == 'in_progress' ? 'warning' : 'success') }} btn-sm">
                                                <i class="bi bi-eye"></i>
                                            </a>
